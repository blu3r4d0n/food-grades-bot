...
weekago = (datetime.now(tz=tz) - timedelta(20)).strftime('%m-%d-%y').replace('-','%2F')
...
for i in df.to_records(index=False):
    conn.execute(text(f"INSERT  OR IGNORE INTO grades(Establishment, Street, City, Grade, Date, Type, PDF, ID, spreadsheet, tweeted, documentcloud) VALUES{i}" ))
